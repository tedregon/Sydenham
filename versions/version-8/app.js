...
function renderTrains(departures) {
    ...
    const p1 = upcoming.filter(d => d.platform === '1').slice(0, SLOT_COUNT);
    const p2 = upcoming.filter(d => d.platform === '2').slice(0, SLOT_COUNT);

    blipsEl.innerHTML = [
        ...p1.map((d, i) => blipMarkup(d, '1', i, p1.length)),
        ...p2.map((d, i) => blipMarkup(d, '2', i, p2.length))
    ].join('');
}

function blipMarkup(d, platform, index, count) {
    const radius = 18 + (Math.min(d.minutes, AXIS_MAX_MIN) / AXIS_MAX_MIN) * 30;
    // Fan trains within each hemisphere so they don't stack on one ray.
    const spread = count <= 1 ? 0 : ((index / (count - 1)) - 0.5) * 70;
    const base = platform === '1' ? 180 : 0;
    const angleDeg = base + spread;
    const angle = (angleDeg * Math.PI) / 180;
    const x = 50 + radius * Math.cos(angle);
    const y = 50 + radius * Math.sin(angle);
    const initials = destinationInitials(d.destination);
    const title = `${d.destination} · ${d.minutes} min · Platform ${platform}`;

    return `<article class="blip" data-operator="${d.operator}" style="left:${x}%; top:${y}%" title="${escapeAttr(title)}">
        <p class="blip-minutes">${d.minutes}</p>
        <div class="blip-dot" aria-hidden="true"></div>
        <p class="blip-dest">${escapeHtml(initials)}</p>
    </article>`;
}
